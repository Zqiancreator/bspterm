...
pub fn initialize_workspace(
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        let keystroke_recording_indicator =
            cx.new(|cx| workspace::KeystrokeRecordingIndicator::new(window, cx));
        ...
        workspace.status_bar().update(cx, |status_bar, cx| {
            ...
            status_bar.add_right_item(keystroke_recording_indicator, window, cx);
...
fn show_software_emulation_warning_if_needed(
    ...
    if specs.is_software_emulated && std::env::var("BSPTERM_ALLOW_EMULATED_GPU").is_err() {
...
mod tests {
    ...
    fn test_bundled_settings_and_themes(cx: &mut App) {
        ...
        for theme_name in themes.list().into_iter().map(|meta| meta.name) {
            ...
            if theme.name.as_ref() == "Bspterm Dark" {

...
/// Protocol type shared by shortcut bar, highlight rules, and other protocol-aware features.
...
pub fn insert_zed_terminal_env(
    ...
    env.insert("BSPTERM_TERM".to_string(), "true".to_string());
    env.insert("TERM_PROGRAM".to_string(), "bspterm".to_string());
...
impl Terminal {
    ...
    pub fn find_number_at_mouse_position(&self, e: &MouseDownEvent) -> Option<number_hover::ParsedNumber> {
        let position = e.position - self.last_content.terminal_bounds.bounds.origin;
        let term = self.term.lock();
        let point = grid_point(
            position,
        ...
        .grid_clamp(&*term, Boundary::Grid);
        number_hover::find_number_at_position(&*term, point)
    }
    ...
    pub fn get_current_protocol(&self) -> Option<TerminalProtocol> {
        match &self.connection_info {
            Some(ConnectionInfo::Ssh { .. }) => Some(TerminalProtocol::Ssh),
            Some(ConnectionInfo::Telnet { .. }) => Some(TerminalProtocol::Telnet),
            None => None,
        }
    }
